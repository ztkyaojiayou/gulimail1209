package com.ztk.gulimall.passport.controller;
...
import com.ztk.gulimall.util.JwtUtil;
...
public class PassportController {
    ...
    public String index(String ReturnUrl, ModelMap map){
        ...
        map.put("ReturnUrl",ReturnUrl);//先保存此回调地址，验证成功之后，再重新请求此地址
        return "index";
    }
    ...
    public String login(UmsMember umsMember, HttpServletRequest request){
        ...
        if(umsMemberLogin!=null){
            ...
            if(StringUtils.isBlank(ip)){
                ip = request.getRemoteAddr();// 从request中获取ip，用于制作盐值
                if(StringUtils.isBlank(ip)){
                    ip = "127.0.0.1";
                }
            }
            ...
            // 同时将token存入redis一份，缓解服务器的压力（很重要）
        ...
        return token;//token制作成功，返回
    }

    //3.认证中心进行校验
    @RequestMapping("verify")
    @ResponseBody
    public String verify(String token,String currentIp){//不是HttpServletRequest request
        ...
        if(decode!=null){
            map.put("status","success");//校验成功，获得用户名和昵称
            map.put("memberId",(String)decode.get("memberId"));
            map.put("nickname",(String)decode.get("nickname"));
        }else{
            map.put("status","fail");//校验失败
        }


        return JSON.toJSONString(map);//把map对象转化为Json字符串，用于前端传输
    }
